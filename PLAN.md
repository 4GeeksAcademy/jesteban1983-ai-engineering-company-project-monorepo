...
- [ ] 1.1 Instalar dependencia `langgraph` con `uv add langgraph`
- [ ] 1.2 Revisar `data/pipelines/rag.py` para entender el retrieve y generate_answer
- [ ] 1.3 Revisar `context-aiAgent` para alinear requisitos
- [ ] 1.4 Crear rama `feature/langgraph-agent-base`
- [ ] **Criterio de éxito**: Repo listo, dependencias instaladas, rama creada

### Fase 2: Grafo del agente (Días 1-2)
- [ ] 2.1 Definir **estado mínimo** del grafo (TypedDict)
- [ ] 2.2 Modelar nodo `receive_question` — validar entrada, detectar vacío
- [ ] 2.3 Modelar nodo `retrieve` — reutilizar `data.pipelines.rag.retrieve`
- [ ] 2.4 Modelar nodo `generate_answer` — reutilizar paso de generación separado
- [ ] 2.5 Modelar nodo `no_info` — respuesta honesta cuando no hay contexto
- [ ] 2.6 Definir **aristas condicionales** (pregunta vacía → END, sin contexto → no_info)
- [ ] 2.7 Compilar el grafo con validación estructural
- [ ] 2.8 Implementar **checkpointing** (MemorySaver)
- [ ] **Criterio de éxito**: Grafo compilado, ejecutable con una pregunta de prueba

### Fase 3: Tracing y evaluación (Día 2)
- [ ] 3.1 Instrumentar tracing (log estructurado o LangSmith)
- [ ] 3.2 Crear `tests/pipelines/test_agent_evals.py`
- [ ] 3.3 Eval 1: ruta feliz — retrieve + generate_answer con contexto válido
- [ ] 3.4 Eval 2: pregunta vacía → ruta directa a END sin recuperar
- [ ] 3.5 Eval 3: sin contexto suficiente → nodo no_info responde con honestidad
- [ ] 3.6 Verificar que tests RAG existentes siguen pasando
- [ ] **Criterio de éxito**: 3 evals pasan + tests RAG existentes pasan

### Fase 4: Endpoint (Día 3)
- [ ] 4.1 Crear `services/api/routes/agent.py` con POST /agent/query
- [ ] 4.2 El endpoint invoca el grafo compilado (sin lógica de negocio propia)
- [ ] 4.3 Manejo de errores: nunca exponer stack trace crudo
- [ ] 4.4 Registrar el router en `main.py`
- [ ] 4.5 Probar endpoint localmente
- [ ] **Criterio de éxito**: POST /agent/query responde con formato JSON correcto

### Fase 5: Documentación y entrega (Día 3)
- [ ] 5.1 Captura o export del trace de una corrida completa
- [ ] 5.2 Resultado de correr los evals
- [ ] 5.3 Pull Request con etiqueta `langgraph-agent-base`
- [ ] 5.4 Actualizar memory/ con resumen del progreso
